"""
Quanti-x web application and REST API server.
...
app.secret_key = os.environ.get("QUANTI_X_SECRET", "quanti-x-local-session-key")

USERS = {
    "officer": {"username": "officer.demo", "password": "Officer@123", "label": "Control Officer"},
    "engineer": {"username": "engineer.demo", "password": "Engineer@123", "label": "Field Engineer"},
}

# Persistent Quanti-x SQLite state and engine instances
database = QuantiXDatabase()
database.initialize_schema()

# Generated local planning data is used only when a user-supplied database is empty.
if not database.has_seed_data():
    ...
    seed_stations = [dict(st, division="Quanti-x Corridor", zone="SUPPLIED", state="Configured") for st in STATIONS]
    database.replace_dataset(seed_stations, seed_sections, seed_tasks, seed_trains, "QUANTI_X_LOCAL")
...
plans_cache = planner.generate_all_horizons(
    tasks, sections, trains, windows,
...
def login():
    selected_role = request.args.get("role", "officer").lower()
    error = None
    if request.method == "POST":
        selected_role = str(request.form.get("role", "officer")).lower()
        account = USERS.get(selected_role)
        ...
        if account and username == account["username"] and password == account["password"]:
            session.clear()
            session.update({"role": selected_role, "username": username, "display_name": account["label"]})
            return redirect(url_for("officer_portal" if selected_role == "officer" else "engineer_portal"))
        error = "The username or password does not match this portal."
    return render_template("login.html", selected_role=selected_role, error=error, demo_users=USERS)
        ...
        def wrapped(*args, **kwargs):
            if session.get("role") != role:
                if request.path.startswith("/api/"):
                    return jsonify({"error": "Authentication required", "login": f"/login?role={role}"}), 401
...
@app.before_request
def protect_runtime_routes():
    if request.path.startswith("/api/") and request.path not in {"/api/portal/summary"} and not session.get("role"):
...
def _station_intelligence(station):
    """Build a database-backed operating profile for one station."""
    code = station["code"]
    section_ids = _station_section_ids(code)
    adjacent_sections = [section for section in sections if section.section_id in section_ids]
    adjacent_trains = {
        train.train_no
        for train in trains
        if any(occupancy["section_id"] in section_ids for occupancy in train.section_occupancies)
    }
    station_tasks = [task for task in tasks if task.station_code == code or task.section_id in section_ids]
    priorities = [float(task.computed_ai_priority or 0.0) for task in station_tasks]
    critical_tasks = sum(priority >= 75.0 for priority in priorities)
    station_windows = [window for window in windows if window.section_id in section_ids]
    best_window = max(station_windows, key=lambda window: window.duration_mins, default=None)
    average_priority = sum(priorities) / len(priorities) if priorities else 0.0
    best_window_minutes = int(best_window.duration_mins) if best_window else 0

    risk_penalty = min(52.0, critical_tasks * 8.0 + average_priority * 0.22)
    traffic_penalty = min(22.0, len(adjacent_trains) * 1.15)
    window_credit = min(24.0, best_window_minutes / 12.0)
    readiness = round(max(0.0, min(100.0, 100.0 - risk_penalty - traffic_penalty + window_credit)), 1)
    if readiness >= 72:
        posture = "READY"
    elif readiness >= 48:
        posture = "CONTROLLED"
    ...
    return {
        "station": station,
        "section_count": len(adjacent_sections),
        "train_count": len(adjacent_trains),
        "task_count": len(station_tasks),
        "critical_task_count": critical_tasks,
        "average_priority": round(average_priority, 1),
        "best_window_minutes": best_window_minutes,
        "best_window": ({**best_window.to_dict(), "start_time": _mins_to_clock(best_window.start_time_mins), "end_time": _mins_to_clock(best_window.end_time_mins)} if best_window else None),
...
def station_analysis():
    ...
    if not station:
        return jsonify({
            "error": "Station not found in the loaded Quanti-x dataset.",
            "query": query,
    ...
    return jsonify({
        ...
        "workflow": workflow,
        "station_intelligence": _station_intelligence(station),
        "workspace_note": "Results use the Quanti-x operational database and supplied corridor data."
    })


@app.route("/api/stations/intelligence", methods=["GET"])
def station_intelligence():
    """Return readiness and workload metrics for every mapped station."""
    profiles = [_station_intelligence(station) for station in station_catalog]
    return jsonify({
        "total_count": len(profiles),
        "stations": profiles,
        "source": "QUANTI_X_DATABASE",
    })


@app.route("/api/ai/block-plan", methods=["POST"])
def ai_block_plan():
    """Predict a practical block strategy for a selected station."""
    payload = request.get_json() or {}
    station = _resolve_station(str(payload.get("station") or ""))
    if not station:
        return jsonify({"error": "Station not found in the Quanti-x database"}), 404

    profile = _station_intelligence(station)
    duration = max(30, min(480, int(payload.get("duration_mins", 120))))
    department = str(payload.get("department") or "TMS").upper()
    urgency = str(payload.get("urgency") or "NORMAL").upper()
    question = str(payload.get("question") or "").strip()
    window = profile["best_window"]
    window_fit = bool(window and duration <= int(window["duration_mins"]))

    if profile["readiness_score"] < 48 or profile["critical_task_count"] >= 3:
        strategy = "Safety-first protected possession"
        recommendation = "Escalate the highest-risk work, protect the next verified window, and avoid combining non-critical work until control confirms capacity."
    elif window_fit:
        strategy = "Bundled joint possession"
        recommendation = "Use the longest verified window and bundle nearby department work into one controlled possession."
    else:
        strategy = "Staged possession"
        recommendation = "Use a shorter staged block, beginning with the highest-priority task, then recheck the timetable before extending it."

    if urgency in {"CRITICAL", "EMERGENCY"}:
        recommendation = "Urgency override: request immediate control protection, then apply this plan as the first feasible safe window. " + recommendation

    confidence = round(max(42.0, min(96.0, profile["readiness_score"] + (14 if window_fit else 0) - (8 if urgency in {"CRITICAL", "EMERGENCY"} else 0))), 1)
    return jsonify({
        "station": profile["station"],
        "question": question,
        "inputs": {"duration_mins": duration, "department": department, "urgency": urgency},
        "prediction": {
            "strategy": strategy,
        ...
        "evidence": {
            "trains_checked": profile["train_count"],
            "maintenance_tasks": profile["task_count"],
            "critical_tasks": profile["critical_task_count"],
            "average_priority": profile["average_priority"],
        },
        "next_steps": [
            "Confirm the selected section and protection limits.",
            "Coordinate the required department resources before sanction.",
            "Recheck train movements immediately before issuing the final block advice.",
        ],
    })
...
def health_check():
    """Lightweight local health endpoint."""
    try:
