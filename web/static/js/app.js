/**
 * Quanti-x - Main Application & State Manager
 */
...
async function loadStationIntelligence() {
    const status = document.getElementById('station-intelligence-status');
    try {
        const response = await fetch('/api/stations/intelligence');
        const data = await response.json();
        appState.stationIntelligence = data.stations || [];
        const selector = document.getElementById('planner-station');
        if (selector) {
            selector.innerHTML = appState.stationIntelligence.map(profile =>
                `<option value="${escapeHtml(profile.station.code)}">${escapeHtml(profile.station.code)} - ${escapeHtml(profile.station.name)}</option>`
            ).join('');
        }
        renderStationReadinessChart();
        if (status) status.textContent = `${appState.stationIntelligence.length} stations scored from Quanti-x database`;
    } catch (error) {
        if (status) status.textContent = 'Station intelligence unavailable';
        console.error('Station intelligence failed:', error);
    }
}

function renderStationReadinessChart() {
    const canvas = document.getElementById('station-readiness-chart');
    const profiles = appState.stationIntelligence || [];
    ...
    const margin = {top: 20, right: 18, bottom: 52, left: 34};
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const barWidth = Math.max(10, chartWidth / profiles.length - 7);
    ...
    profiles.forEach((profile, index) => {
        ...
        const x = margin.left + index * (chartWidth / profiles.length) + 3;
        const barHeight = (score / 100) * chartHeight;
        context.fillStyle = score >= 72 ? '#2f8f83' : score >= 48 ? '#c88b2e' : '#bd5b5b';
        context.fillRect(x, margin.top + chartHeight - barHeight, barWidth, barHeight);
        context.save();
        context.translate(x + barWidth / 2, height - 12);
        context.rotate(-Math.PI / 4);
        context.fillStyle = '#42526a';
        context.fillText(profile.station.code, 0, 0);
        context.restore();
    });
}

async function runBlockPlanner() {
    const result = document.getElementById('planner-result');
    const station = document.getElementById('planner-station')?.value;
    const department = document.getElementById('planner-department')?.value;
    const duration = Number(document.getElementById('planner-duration')?.value || 120);
    const urgency = document.getElementById('planner-urgency')?.value;
    const question = document.getElementById('planner-question')?.value || '';
    if (!result || !station) return;
    result.textContent = 'Calculating a database-grounded block strategy...';
    try {
        const response = await fetch('/api/ai/block-plan', {
            method: 'POST', headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({station, department, duration_mins: duration, urgency, question})
        });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || 'Planner failed');
        document.getElementById('planner-confidence').textContent = `${data.prediction.confidence}% confidence`;
        result.innerHTML = `<div class="font-bold text-slate-900">${escapeHtml(data.prediction.strategy)}</div>
            <p class="mt-1">${escapeHtml(data.prediction.recommendation)}</p>
            <div class="mt-2 grid grid-cols-2 gap-2 text-[11px]"><span>Readiness <strong>${data.prediction.readiness_score}/100</strong></span><span>Window fit <strong>${data.prediction.window_fit ? 'Yes' : 'No'}</strong></span><span>Train movements <strong>${data.evidence.trains_checked}</strong></span><span>Critical tasks <strong>${data.evidence.critical_tasks}</strong></span></div>`;
    } catch (error) {
    ...
    if (tbody && comp) {
        tbody.innerHTML = `
            <tr class="hover:bg-slate-900/40 transition">
                ...
                <td class="py-3 px-4 text-emerald-700 font-mono font-bold">${comp.block_utilization_pct.quanti_x}% (Unified)</td>
            ...
            <tr class="hover:bg-slate-900/40 transition">
                ...
                <td class="py-3 px-4 text-emerald-700 font-mono font-bold">${comp.total_track_downtime_hours.quanti_x} Hours</td>
            ...
            <tr class="hover:bg-slate-900/40 transition">
                ...
                <td class="py-3 px-4 text-emerald-700 font-mono font-bold">${comp.train_punctuality_loss_mins.quanti_x} Mins</td>
            ...
            <tr class="hover:bg-slate-900/40 transition">
                ...
                <td class="py-3 px-4 text-emerald-700 font-mono font-bold">${comp.asset_availability_pct.quanti_x}%</td>
            ...
            <tr class="hover:bg-slate-900/40 transition">
                ...
                <td class="py-3 px-4 text-violet-700 font-mono font-bold">${comp.shadow_blocking_rate_pct.quanti_x}%</td>
            ...
            <tr class="hover:bg-slate-900/40 transition">
                ...
                <td class="py-3 px-4 text-emerald-700 font-mono font-bold">${comp.critical_safety_compliance_pct.quanti_x}%</td>
...
// ----------------------------------------------------
// Station-level Quanti-x analysis
// ----------------------------------------------------
